...
### G1 ####
  ...
  A_Mat <- 2 * kinship(id = rownames(Pedigree_All),
                       dadid = Pedigree_All$father,
                       momid = Pedigree_All$mother)
  ...
  # Should I use the same initial error variance here?
    ...
    identical(colnames(G0_Pheno_mean), colnames(New_Pheno_mean))  # Should be TRUE
    nrow(G0_Pheno_mean)
    nrow(New_Pheno_mean)
    nrow(GLOBAL_Phenotypes)  # Should equal the sum of the above
    ...
    cor(EBVs$V1,GLOBAL_Phenotypes$Trait1_F)
    
    sum(is.na(EBVs$V1))  # Count of NAs in EBVs$V1
    sum(is.na(GLOBAL_Phenotypes$Trait1_F))  # Count of NAs in Trait1_F
    
    
    dim(EBVs)
    dim(GLOBAL_Phenotypes)
