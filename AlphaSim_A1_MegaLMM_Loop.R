...
G0_pop <- setPheno(G0_pop, varE = initVarE, corE =  matrix(c(1, CorP,
                                           CorP, 1), nrow=2),rep = 1, simParam = NULL, onlyPheno = FALSE)
...
MegaLMM_state <- run_megalm_analysis(
  ...
  burn_iter = 50, sample_iter = 50, n_burn_runs = 5, n_sample_runs = 5,
...
#  G = A.mat(M_All, impute.method = "mean", min.MAF = 0.01)
  ...
  # Set phenotypes
  pop <- setPheno(pop, rep = 10, varE = initVarE, corE =  matrix(c(1, CorP,
                                                                   CorP, 1), nrow=2), simParam = NULL, onlyPheno = FALSE)
  Pheno <- as.data.frame(pop@pheno)
  Pheno$ID <- pop@id
...
A <- 2 * kinship(id = as.numeric(rownames(Pedigree_All)),
  ...
  # Select and cross for next generation (now families exist)
  Parents <- selectWithinFam(pop, nInd = 1, use = "ebv", selectTop = TRUE)
...
results_trait1$Generation <- as.numeric(results_trait1$Generation)
results_trait2$Generation <- as.numeric(results_trait2$Generation)
wf_trait1$Generation <- as.numeric(wf_trait1$Generation)
wf_trait2$Generation <- as.numeric(wf_trait2$Generation)
...
p3 <- ggplot() +
  ...
  geom_point(data = results_trait1, 
             ...
             size = 2) +
  geom_line(data = results_trait1, 
            aes(x = factor(Generation), y = Accuracy, color = "EBV", group = 1), 
            size = 1.2) +
  geom_point(data = results_trait1, 
             aes(x = factor(Generation), y = Accuracy, color = "EBV"), 
             size = 2) +
  labs(y = "Accuracy", x = "Generation", title = "Trait1: Accuracy (EBV vs WF)") +
  ylim(0,1) +
  scale_color_manual(values = c("EBV" = "purple", "WF" = "magenta")) +
  theme_minimal() +
  theme(legend.position = "bottom")
# ---------------- Trait2 Plots ---------------- #
# GV vs Phenotype
